...
exports.getMainCategory = (req,res) =>{
  ...
  MainCategories.find({}).limit(2).skip(req.query.page * 2).sort(sort).exec(function(err, result) {res.json(result)})
}

exports.getCategory = (req,res) =>{
  sort = sortAndLimit(req)
  Categories.find({main_category: req.params.maincategory_id}).limit(2).skip(req.query.page * 2).sort(sort).exec(function(err, result) {res.json(result)})
}

exports.getSubCategory = (req,res) =>{
  sort = sortAndLimit(req)
  SubCategories.find({category: req.params.category_id}).limit(2).skip(req.query.page * 2).sort(sort).exec(function(err, result) {res.json(result)}
  )
}
